...
const Cart = dynamic(() => import("../components/header/Cart"), {
  ssr: false,
  loading: () => <div className="w-[40px] h-[32px]"><Skeleton variant="rectangular" width={40} height={32} /></div>,
});
const User = dynamic(() => import("@/components/header/User"), {
  ssr: false,
  loading: () => <div className="w-[91.88px] h-[38.06px]"><Skeleton variant="rectangular" width={91.88} height={38.06} /></div>,
});
...
const Menu = dynamic(() => import("@/components/menu/Menu"), {
  ssr: false,
  loading: () => <div className="w-[128.69px] h-[22px] sm:block hidden"><Skeleton variant="rectangular" width={128.69} height={22} /></div>,
});
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Logo className="col-span-1" logo={dataSetting && dataSetting.data.data.setting.logo} />
          :
          <div className="h-[38.06px] w-full col-start-2">
            <Skeleton variant="rectangular" sx={{width:'100%'}} height={38.06} animation="wave" />
          </div>
          }
          ...
          <div className="xl:col-span-2 col-span-3 sm:flex hidden justify-end gap-4 items-center h-full">
            ...
            <a
              href={`tel:${dataSetting && dataSetting.data.data.phones.top_mobile_phones}`}
              className="text-white flex col-span-2 text-lg"
            >
              <div className="lg:text-lg text-sm">{dataSetting && dataSetting.data.data.phones.top_mobile_phones}</div>
              <PhoneInTalkIcon className="mr-1" />
            </a>
            :
            <div className="w-[93.41px] h-[28px]"><Skeleton variant="rectangular" width={93.41} height={28} /></div>
          }
        ...
        {/* {passName === '/product' && (<QuickTabs title={["خلاصه توضیحات" , "مشخصات محصول" , "پرسش و پاسخ" , "محصولات مشابه" , "نقد و بررسی"]} selectValue={selected}/>)} */}
        {/* {!mobile && <SearchBox shownSearchBox={open} />} */}
      </header>
      {tablet && (
        <>
          <MobileMenu
...
{
  /* <div className="h-[65px] w-full bg-gray-300 rounded-sm animate-pulse dark:bg-gray-700 ml-2"></div> */
}
